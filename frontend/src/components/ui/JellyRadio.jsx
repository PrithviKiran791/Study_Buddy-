...
const SIZES = { sm: [28, 11, 12], md: [36, 13, 16], lg: [44, 14, 20] };
...
export default function JellyRadio({
  ...
  const rawList = typeof items === 'string' && items === 'levels'
    ? DEFAULT_ITEMS
    : Array.isArray(items)
      ? items
      : DEFAULT_ITEMS;

  const list = rawList.map((it) => (typeof it === 'string' ? { value: it, label: it } : it));
